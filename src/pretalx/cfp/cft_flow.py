...
def cft_session(request):
    request.session.modified = True
    if "cft" not in request.session or not request.session["cft"]:
        request.session["cft"] = {}
    key = request.resolver_match.kwargs["tmpid"]
    if key not in request.session["cft"]:
        request.session["cft"][key] = {
            "data": {},
            "initial": {},
            "files": {},
        }
    return request.session["cft"][key]
...
class BaseCfTStep:
    ...
    @cached_property
    def cft_session(self):
        return cft_session(self.request)
...
class TemplateFlowStep(TemplateResponseMixin, BaseCfTStep):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs.setdefault(
            ...
            [
                step
                for step in self.event.cft_flow.steps
...
class FormFlowStep(TemplateFlowStep):
    ...
    def get_files(self):
        saved_files = self.cft_session["files"].get(self.identifier, {})
        files = {}
    ...
    def set_files(self, files):
        for field, field_file in files.items():
            ...
            data = self.cft_session["files"].get(self.identifier, {})
            data[field] = file_dict
            self.cft_session["files"][self.identifier] = data


class GenericFlowStep:
    @cached_property
    def config(self):
        return self.event.cft_flow.config.get("steps", {}).get(self.identifier, {})
...
class InfoStep(GenericFlowStep, FormFlowStep):
    ...
    @property
    def label(self):
        return _("Track Information")

    @property
    def _title(self):
        return _("Call for Open Source Community Participation")

    @property
    def _text(self):
        return _(
            "We are delighted to announce that we are currently inviting submissions for Community Room."
        )
...
class ContactStep(GenericFlowStep, FormFlowStep):
    ...
    def get_form_kwargs(self):
        result = super().get_form_kwargs()
        user_data = copy.deepcopy(self.cft_session.get("data", {}).get("user", {}))
    ...
    def get_context_data(self, **kwargs):
        ...
        if email is None:
            data = self.cft_session.get("data", {}).get("user", {})
...
class CfTFlow:
    ...
    def save_config(self, data):
        ...
        self.event.settings.cft_flow = data
