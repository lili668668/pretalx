...
class JoinStartView(EventPageMixin, View):
    @staticmethod
    def get(request, *args, **kwargs):
        url = reverse(
            "cfp:event.join",
            kwargs={
                "event": request.event.slug,
                "step": list(request.event.cft_flow.steps_dict.keys())[0],
                "tmpid": get_random_string(length=6),
            },
        )
        if request.GET:
...
class JoinWizard(EventPageMixin, View):
    @transaction.atomic
    def dispatch(self, request, *args, **kwargs):
        self.event = self.request.event
        ...
        for step in request.event.cft_flow.steps:
            if not step.is_applicable(request):
                continue
            if step.identifier == kwargs["step"]:
                break
    ...
    def done(self, request):
        ...
        # We are done, or at least the data checks out. Time to save results.
        request.event.cft_flow.steps_dict["user"].done(request)
        for step in valid_steps:
            if not step.identifier == "user":
                step.done(request)

        try:
            request.event.ack_track_template.to_mail(
                user=request.user,
                context={"track_name": request.track.name, "track_url": request.event.orga_urls.cft},
                context_kwargs={"user": request.user},
                skip_queue=True,
